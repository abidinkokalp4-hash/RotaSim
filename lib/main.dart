...
class _Home extends State<Home>{
 ...
 bool sat=false,drawing=true,playing=false,smooth=true,freehandActive=false,autoEnd=true,routeFinished=false; LatLng? me; int playIndex=0; Timer? timer;
 ...
 void exportSheet(){showModalBottomSheet(context:context,isScrollControlled:true,builder:(x)=>SafeArea(child:Padding(padding:const EdgeInsets.all(18),child:Wrap(runSpacing:8,children:[Text('Dışa Aktar',style:Theme.of(context).textTheme.headlineSmall),const SizedBox(width:500),for(final e in const [('gpx-track','GPX Track',Icons.timeline),('gpx-route','GPX Route',Icons.route),('kml','KML',Icons.public),('geojson','GeoJSON',Icons.data_object),('csv','CSV',Icons.table_chart),('json','JSON',Icons.code),('txt','TXT',Icons.description),('bin','BIN',Icons.memory),('rotasim','RotaSim Yedeği',Icons.backup)])SizedBox(width:MediaQuery.sizeOf(context).width/2-24,child:OutlinedButton.icon(onPressed:(){Navigator.pop(x);shareType(e.$1);},icon:Icon(e.$3),label:Text(e.$2))),SizedBox(width:double.infinity,child:FilledButton.icon(onPressed:(){Navigator.pop(x);shareZip();},icon:const Icon(Icons.folder_zip),label:const Text('Tümünü ZIP Olarak Paylaş')))]))));}
 ...
 @override Widget build(BuildContext c){final fmt=DateFormat('dd.MM.yyyy HH:mm:ss');return Scaffold(body:Stack(children:[
  Listener(onPointerDown:(e){if(drawing){freehandActive=true;freehandPoint(e.localPosition);}},onPointerMove:(e)=>freehandPoint(e.localPosition),onPointerUp:(_)=>freehandActive=false,onPointerCancel:(_)=>freehandActive=false,child:FlutterMap(mapController:mc,options:MapOptions(initialCenter:const LatLng(39.93,32.86),initialZoom:12,onTap:(_,p)=>add(p),onLongPress:(_,p){if(pts.isEmpty)return;int best=0;double bd=1e99;for(int i=0;i<pts.length;i++){final d=D(p,pts[i]);if(d<bd){bd=d;best=i;}}stopAt(best);}),children:[
   if(!sat)const MapLibreLayer(initStyle:'https://tiles.openfreemap.org/styles/liberty'),
   if(sat)TileLayer(urlTemplate:'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',userAgentPackageName:'com.rotasim.rotasim'),
  ...
  SafeArea(child:Padding(padding:const EdgeInsets.all(10),child:Row(children:[FilledButton.tonalIcon(onPressed:()=>setState(()=>sat=!sat),icon:Icon(sat?Icons.map:Icons.satellite_alt),label:Text(sat?'Harita':'Uydu')),const SizedBox(width:6),IconButton.filledTonal(tooltip:'Çizim modu',onPressed:()=>setState((){drawing=!drawing;if(drawing)routeFinished=false;}),icon:Icon(drawing?Icons.edit:Icons.pan_tool)),IconButton.filledTonal(tooltip:'Yumuşatma',onPressed:()=>setState(()=>smooth=!smooth),icon:Icon(smooth?Icons.gesture:Icons.polyline)),const Spacer(),IconButton.filledTonal(onPressed:locate,icon:const Icon(Icons.my_location)),IconButton.filledTonal(onPressed:saved,icon:const Icon(Icons.folder))]))),
  Positioned(left:10,right:10,bottom:10,child:Card(child:Padding(padding:const EdgeInsets.all(12),child:Column(mainAxisSize:MainAxisSize.min,children:[
   Row(children:[Expanded(child:Text('Rota Kontrolü',style:Theme.of(context).textTheme.titleMedium)),const Spacer(),IconButton(onPressed:back,icon:const Icon(Icons.undo)),IconButton(onPressed:forward,icon:const Icon(Icons.redo)),IconButton(onPressed:clearRoute,icon:const Icon(Icons.delete_outline))]),
   ...
   Text(routeFinished?'Rota tamamlandı • Düzenlemek için kalemi aç':drawing?'Serbest çizim • Parmağını sürükleyerek rotayı çiz • Uzun bas: durak':'Kaydırma modu • Kalem simgesinden çizimi aç',style:const TextStyle(fontSize:11))
  ]))))
 ]));}
}