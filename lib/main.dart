...
class _Home extends State<Home>{
 ...
 bool sat=false,drawing=false,playing=false,smooth=true,freehandActive=false,autoEnd=true,routeFinished=false; int activePointers=0; LatLng? me; int playIndex=0; Timer? timer;
 ...
 @override Widget build(BuildContext c){final fmt=DateFormat('dd.MM.yyyy HH:mm:ss');return Scaffold(body:Stack(children:[
  ...
  SafeArea(child:Padding(padding:const EdgeInsets.all(12),child:Row(children:[FilledButton.tonalIcon(onPressed:()=>setState(()=>sat=!sat),icon:Icon(sat?Icons.map_outlined:Icons.satellite_alt_outlined),label:Text(sat?'Yol':'Uydu')),const Spacer(),IconButton.filledTonal(onPressed:locate,tooltip:'Konumum',icon:const Icon(Icons.my_location)),const SizedBox(width:6),IconButton.filledTonal(onPressed:saved,tooltip:'Kayıtlı rotalar',icon:const Icon(Icons.folder_outlined))]))),
